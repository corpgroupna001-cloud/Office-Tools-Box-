    ...
    function renderWeek(items, days) {
        ...
        const cols = days.map(d => {
            ...
            const evs = placed.map(p => {
                ...
                // Two side by side; three or more cascade (each 60% wide, offset) so every label stays readable.
                const geo = lanes <= 2 ? { left: p.lane * w, width: w } : { left: p.lane * (40 / (lanes - 1)), width: 60 };
                const drag = it.kind === 'event' && canEditEvent(it.ev) && it.ev.status !== 'cancelled';
                return `<div class="cal-ev timed${short ? ' short' : ''}${it.cls || ''}${drag ? ' can-drag' : ''}" data-kind="${it.kind}" data-item="${esc(it.key)}" role="button" tabindex="0" title="${esc(`${L.fmtTime(it.startIso)} · ${it.tip || it.title}`)}" style="--ev:${colorOf(it)};top:${p.s / 60 * HOUR_PX}px;height:${Math.max(22, (p.e - p.s) / 60 * HOUR_PX - 2)}px;left:calc(${geo.left}% + 2px);right:auto;width:calc(${geo.width}% - 4px);z-index:${3 + p.lane}">${short && lanes > 1 ? '' : `<span class="tm">${esc(L.fmtTime(it.startIso))}${p.e - p.s >= 90 && it.endIso && it.kind === 'event' ? ' – ' + esc(L.fmtTime(it.endIso)) : ''}</span>`}<span class="tt">${esc(it.title)}</span>${drag && !(it.ev.all_day) ? '<span class="rz" aria-hidden="true"></span>' : ''}</div>`;
            }).join('');
