...
function employeePatch(body, current) {
  ...
  // Employee ID: the ID a person is known by (employee_code is the biometric one).
  // Left out when empty and the database has no such column yet, so adding and
  // editing employees keeps working until supabase-employee-id-migration.sql runs.
  if ('employee_id' in body) {
    if (body.employee_id != null && typeof body.employee_id !== 'string') fail('Invalid employee_id');
    const value = String(body.employee_id || '').trim();
    if (value.length > 50) fail('Employee ID is too long');
    if (value || 'employee_id' in current) patch.employee_id = value || null;
  }
...
/** Which value is already taken, from the unique index Postgres names. */
function duplicateMessage(detail) {
  const text = `${detail.message || ''} ${detail.details || ''}`;
  if (/employee_id/.test(text)) return 'That Employee ID already belongs to another employee';
  if (/employee_code/.test(text)) return 'That biometric ID already belongs to another employee';
  return 'Email, Employee ID or biometric ID already belongs to another employee';
}
...
async function updateEmployee(body, { url, key, request = fetch }) {
  ...
  if (!pr.ok) {
    ...
    if (detail.code === '23505') fail(duplicateMessage(detail), 409);
    if ((detail.code === 'PGRST204' || detail.code === '42703') && 'employee_id' in patch) fail('Apply supabase-employee-id-migration.sql before setting Employee IDs', 409);
