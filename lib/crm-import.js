...
function mapDeals(rows, ctx = {}) {
  ...
  (rows || []).forEach((r, i) => {
    ...
    out.rows.push({
      ...
      // The responsible employee's company, else the one chosen for the import:
      // a record with no company is seen by nobody but workspace admins.
      company: (person && person.company) || ctx.company || null,
