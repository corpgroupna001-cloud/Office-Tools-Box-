...
function hookBaseFor(enroll) {
  const key = 'BITRIX_HOOK_' + String(enroll || '').trim();
 ...
 */
async function callAt(base, method, params = {}) {
  if (!base) {
    return { ok: false, reason: 'not_configured',
...
async function listConversations() {
  ...
  for (let page = 0; page < 3; page++) {
    ...
    const r = await call('im.recent.get', params);
 ...
 */
async function whoAmI(base = webhookBase()) {
...
module.exports = { call, callAt, sendGroupMessage, sendAndLog, logAttempt,
                   listGroups, listConversations, whoAmI, isConfigured, hookBaseFor, redact };
