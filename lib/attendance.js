...
/**
 * Which of a dual-shift person's two windows an instant falls in: 2 once the
 * clock has reached the second shift's start on a day it covers, else 1.
...
function shiftPartAt(shift, shift2, when) {
  ...
  if (!shiftDays(shift2).includes(weekday)) return 1;
  if (!shift) return 2;
  const t = timeToMinutes(istParts(d).isoTime);
  const s1 = timeToMinutes(shift.start_time), s2 = timeToMinutes(shift2.start_time);
  const rel = (from, to) => (((to - from) % 1440) + 1440) % 1440;
  // Measured from the first shift's start so an overnight first shift still
  // orders correctly: the second window begins once we pass its start.
  return rel(s1, t) >= rel(s1, s2) ? 2 : 1;
}
