 ...
 *
 * So punches are grouped into days by the GAP between them, not by the
 * clock: a punch that lands within NEW_DAY_GAP_HOURS of the person's
 * previous punch belongs to the same day as that punch; a longer silence
 * starts a new day, dated by the calendar date the new day began on. A day
 * shift (09:30-18:30, back at 09:30) and a night shift (18:00-03:00, back at
 * 18:00) both leave a 14-15 hour gap between days; the longest break inside a
 * day is a few hours. Eight hours sits comfortably between the two, and it
 * needs no shift to be assigned - which matters, because a punch from someone
 * not yet mapped to a profile has no shift to consult.
 *
...
/**
 * Which attendance date a punch belongs to.
 *
 ...
 * day_started_at } in chronological order. Punches that already carry a real
 * direction (a device that sends one, or a selfie event) keep it.
 *
 * Used at ingest for the batch plus the stored context around it, and by
 * the backfill in the admin tab, so both apply exactly the same rules.
 */
function assignDays(punches, { shiftFor } = {}) {
  const list = (punches || []).slice()
    .sort((a, b) => new Date(a.log_datetime) - new Date(b.log_datetime));
  let prev = null;          // previous punch, with its computed log_date
  let day = null;           // { date, startedAt, count }
  const out = list.map(p => {
    const date = attendanceDateFor(p.log_datetime, prev);
    if (!day || day.date !== date) day = { date, startedAt: p.log_datetime, count: 0 };
    const priors = day.count;
    let direction = p.direction;
    let derived = !!p.direction_derived;
    if (!direction || direction === 'UNKNOWN' || derived) {
      // Parity within the attendance day: 1st = IN, 2nd = OUT, 3rd = IN ...
      direction = priors % 2 === 0 ? 'IN' : 'OUT';
      derived = true;
    }
    const shift = typeof shiftFor === 'function' ? shiftFor(p) : null;
    const event_type = p.source === 'selfie' && p.event_type ? p.event_type : deriveEventType({
      priorsToday: priors, direction, when: p.log_datetime, shift,
      dayStartedAt: day.startedAt,
    });
    day.count += 1;
    const row = { ...p, log_date: date, direction, direction_derived: derived, event_type,
                  priors, day_started_at: day.startedAt };
    prev = row;
    return row;
  });

  // Second pass, with hindsight. deriveEventType guesses at punch time
  // whether an OUT is the last of the day; once a LATER punch exists on the
  // same day, that guess is settled: it was a break. Only the day's final OUT
  // keeps the shift-end guess, because for that one the future is still
  // unknown. Selfie punches named their own event and are left alone.
  for (let i = 0; i < out.length; i++) {
    const r = out[i];
    if (r.direction !== 'OUT' || r.source === 'selfie') continue;
    const next = out[i + 1];
    const sameDayLater = !!next && next.log_date === r.log_date;
    const dayClosed    = !!next && next.log_date !== r.log_date;   // a later day exists
    if (sameDayLater && r.event_type === 'LOGOUT') r.event_type = 'BREAK_OUT';
    // The day's last punch, and the person has since started another day:
    // that OUT was the Logout however early it looked at the time.
    if (dayClosed && r.event_type === 'BREAK_OUT') r.event_type = 'LOGOUT';
  }
  return out;
}
...
/**
 * A day nobody closed, once its shift is over. Someone who logged in (or came
 ...
 * or was closed properly.
 *   kind  'no_punch_out'        at = the shift end
 *         'break_not_returned'  at = the break-out punch
 */
function autoLogoutFor({ date, shift, punches, now }) {
  const list = (punches || []).slice().sort((a, b) => new Date(a.log_datetime) - new Date(b.log_datetime));
  const last = list[list.length - 1];
  if (!last) return null;
  const end = shiftEndAt(date, shift);
  if (!end) return null;
  if ((now ? new Date(now) : new Date()) < end) return null;          // the shift is still on
  if (last.direction === 'IN') {
    // A login or break-in after the shift ended is overtime still going on.
    if (new Date(last.log_datetime) > end) return null;
    return { kind: 'no_punch_out', at: end, lastPunch: last };
  }
...
function classifyDay({ date, shift, shift2, weekOffs, holiday, leave, punches, today, now }) {
  ...
  if (shift2) shift = effectiveShift(shift, shift2, weekday);
  const list = (punches || []).slice()
    .sort((a, b) => new Date(a.log_datetime) - new Date(b.log_datetime));
