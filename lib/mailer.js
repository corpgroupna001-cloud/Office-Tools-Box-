...
function transportFor(user) {
  ...
  transporterCache[user] = nodemailer.createTransport({
    ...
    // cPanel shared mail hosts often present a certificate for the server's
    // own name, not the mail domain. SMTP_TLS_STRICT=1 verifies it (set
    // SMTP_TLS_SERVERNAME to the name on the certificate); without it the
    // certificate is not checked, as before, which a network attacker could
    // exploit to read SMTP_PASS. Turn it on once a test mail goes through.
    tls: process.env.SMTP_TLS_STRICT === '1'
      ? { rejectUnauthorized: true, servername: process.env.SMTP_TLS_SERVERNAME || process.env.SMTP_HOST }
